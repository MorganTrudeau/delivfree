...
function updateAppJson() {
  ...
  const newVersionInfo = incrementVersion(appJson.version, appJson.buildNumber);
  ...
  console.log(
    `Updated app.json to version ${appJson.version}, build ${appJson.buildNumber}`,
  );
}
...
function updateIosInfoPlist() {
  const infoPlistPath = './ios/delivfree/Info.plist';
  ...
  console.log(
    `Updated ios/delivfree/Info.plist to version ${newVersionInfo.version}, build ${newVersionInfo.buildNumber}`,
  );
}
